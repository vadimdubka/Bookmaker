...
public class ValidatorServiceImpl implements ValidatorService {
    ...
    public boolean isValidEmail(String email) {
        ...
        if ((email != null) && (!email.isEmpty()) && (email.length() <= MAX_EMAIL_LENGTH) && (isMatchPattern(email, EMAIL_REGEX))) {
            ...
            if (emailPair.length == EMAIL_PAIR_LENGTH) {
                ...
                isValid = (name.length() <= MAX_EMAIL_NAME_LENGTH) && (domain.length() <= MAX_EMAIL_DOMAIN_LENGTH);
            }
        }
        return isValid;
    }
